...
class UserSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if "is_anonymous" in data and data["is_anonymous"]:
            if "password" in data:
                raise serializers.ValidationError({"password": "This field is not required for anonymous user."})
            if "email" in data:
                raise serializers.ValidationError({"email": "This field is not required for anonymous user."})
            data["email"] = None
            data["password"] = None
    ...
    def create(self, validated_data):
        ...
        if email is None and password is None:
            return User.objects.create_user(email=None, password=None, is_anonymous=True)
        else:
    ...
    def send_confirmation_email(user_email, activation_link):
        ...
        logo_url = "https://todo.com/logo.png"
...
class GroupSerializer(serializers.ModelSerializer):
    users = serializers.PrimaryKeyRelatedField(many=True, read_only=True, required=False)
