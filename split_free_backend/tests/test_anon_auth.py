...
from split_free_backend.core.models import Group, Member, User
...
class AnonUserTests(TestCase):
    ...
    def test_anon_user_can_fetch_data(self):
        ...
        response = self.client.get(url, format="json", headers=get_auth_headers(access_token))
    ...
    def test_anon_user_can_attach_to_group_member(self):
        anon_user = User.objects.create(email=None, password=None, is_anonymous=True, is_active=True)
